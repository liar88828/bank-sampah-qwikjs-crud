...
export const useLoadPenyerahan = routeLoader$(async ({ sharedMap, query }) => {
  ...
  const res = await transaksi.findPenyerahanTransaksi(id, page, search);
  console.log(res);
  return res;
});
...
export const Heads = component$(() => {
  return <Breadcrumbs data={getBreadcrumbTrail("Table Penyerahan")} />;
});
                    ...
                    {data.map((d, i) => (
                      <tr key={d.id}>
                        ...
                        <td class="flex flex-nowrap gap-2">
                          <Link href={`${d.id}`} class="btn btn-primary btn-xs">
                            Detail
                          </Link>
                        </td>
                      </tr>
