...
export const useGetUserId = routeLoader$(
  async ({ sharedMap, params, redirect }) => {
    const session = sharedMap.get("session") as Session
    if (session.user.email?.includes("liar")) {
      return db.users.findUser(params["userId"])
    } else {
      throw redirect(303, "/")
    }
  },
            ...
            <div class="grid w-full grid-cols-2 gap-2 text-left">
              <label class="form-control">
                ...
                <input
                  ...
                  value={
                    data?.tempat_lahir ??
                    dataUpdate.formData?.get("tempat_lahir")
                  }
                />
                {dataUpdate.value?.failed && (
                  <p class="text-xs text-error">
            ...
            <div class="grid w-full grid-cols-1 gap-2 text-left">
              <label class="form-control">
                ...
                <textarea
                  ...
                  minLength={10}
                  value={
                    data?.alamat ?? "" ?? dataUpdate.formData?.get("alamat")
                  }
                ></textarea>
                {dataUpdate.value?.failed && (
                  <p class="text-xs text-error">
            ...
            <div class="grid w-full grid-cols-2 gap-2 text-left">
              <label class="form-control ">
                ...
                <select
                  ...
                  class="input input-bordered"
                  value={
                    data?.bahasa ?? dataUpdate.formData?.get("bahasa") ?? ""
                  }
                >
                  <option disabled selected>
                    Pilih Bahasa
