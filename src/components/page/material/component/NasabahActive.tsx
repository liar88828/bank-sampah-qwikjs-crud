...
import { useActionNasabah } from "~/action/page.action"
import type { NasabahLoader } from "~/type/db/menu.type"

export const NasabahActive = component$(({ data }: { data: NasabahLoader }) => {
          ...
          <tbody>
            {data.searchNasabah.map((d, i) => (
              <tr key={d.nama}>
                ...
                <td>{d.email}</td>
                <td>
                  <Link href={`user/${d.id}`} class="btn btn-info  btn-sm">
                    details
                  </Link>
                </td>
              </tr>
